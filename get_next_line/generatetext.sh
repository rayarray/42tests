...
echo "Generating $1 files with $2 lines and $3 max line width, slow method";
...
do
	...
	touch "./tsttxt$i";
		perl -MList::Util -e 'print List::Util::shuffle <>' /usr/share/dict/words | \
		awk 'NR <= $(( ($2 * $3)  / 7 )) { print  }' | fmt -w $3 | sed -n $(eval echo "'1,$2p;'") >> ./tsttxt$i;
#	for j in $(eval echo "{1..$2}")
#	do
#		perl -MList::Util -e 'print List::Util::shuffle <>' /usr/share/dict/words | \
#			awk 'NR <= $(( $3 / 7 )) { print  }' | fmt -w $3 | sed -n '1,$2p;' >> ./tsttxt$i;
#	done
done
fi
